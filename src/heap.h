...
#include <sstream>
     ...
     * 应始终为true。例如用std::less则为最大堆（最大的元素在顶部）
     *
     */
    namespace slib {
  template <typename T, typename Compare,
            typename Allocator = class MallocAllocator>
  class Heap {
   public:
    ...
    iterator begin() { return storageBegin; }

    const_iterator begin() const { return storageBegin; }

    iterator end() { return dataEnd; }

    const_iterator end() const { return dataEnd; }

    size_type getCapacity() const { return storageEnd - storageBegin; }

    size_type getSize() const { return dataEnd - storageBegin; }
    ...
    int outputAsGraph(const char* filename) {
      ...
      Agnode_t** nodes =
          (Agnode_t**)Allocator::malloc(sizeof(Agnode_t*) * nodesNumber);
   ...
   private:
    ...
    T& get(size_type i) const { return *(this->storageBegin + i); }
