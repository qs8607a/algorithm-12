...
public:	
	void getRandomString(int length,std::string& ret){
		static const char* allchars="abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
...
TEST_F(BloomFilterTest, Test1) {
  ...
  BloomFilter<std::string>* bf=BloomFilter<std::string>::create(keys,keysCount);
  int matchCount=0;
  for(int i=0;i!=keysCount;++i){
