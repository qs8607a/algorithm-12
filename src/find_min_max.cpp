...
std::pair<Iterator, Iterator> find_min_max_brute_force(Iterator begin,
                                                       Iterator end,
                                                       Comp greater) {
  Iterator max = begin;
  Iterator min = begin;
...
template <typename Iterator, typename Comp>
std::pair<Iterator, Iterator> find_min_max2(Iterator begin, Iterator end,
                                            Comp greater) {
  if (begin + 1 >= end)
...
static void test1() {
  ...
  auto ret = slib::find_min_max_brute_force(randomInt.begin(), randomInt.end(),
                                            std::greater<int>());
...
#define MESURE_TIME_END(PREFIX)                                                \
  {                                                                            \
    ...
    std::cout << "total time=" << diff.tv_sec << "秒" << diff.tv_usec         \
              << "微秒" << std::endl;                                        \
  }

static std::string init_openssl_rand() {
  char seed_filename[4096];
  if (!RAND_file_name(seed_filename, sizeof(seed_filename) - 1))
    return "";
...
static void test2() {
  ...
  auto ret = slib::find_min_max_brute_force(
      numbers.get(), numbers.get() + count, [](uint64_t l, uint64_t r) {
    return l < r;
  });
  MESURE_TIME_END(T1)
  ...
  auto ret2 = std::minmax_element(numbers.get(), numbers.get() + count,
  ...
  auto ret3 = slib::find_min_max2(numbers.get(), numbers.get() + count,
                                  [](uint64_t l, uint64_t r) {
    return l < r;
  });
  MESURE_TIME_END(T4)
...
static void test3() {
  ...
  ;
  MESURE_TIME_BEGIN(T1)
  auto ret = slib::find_min_max_brute_force(numbers.get(),
                                            numbers.get() + count, comp);
  ...
  MESURE_TIME_BEGIN(T2)
  std::string max =
      *std::max_element(numbers.get(), numbers.get() + count, comp);
  std::string min =
      *std::min_element(numbers.get(), numbers.get() + count, comp);
  std::cout << "comp_count=" << comp_count << std::endl;
  comp_count = 0;
  MESURE_TIME_END(T2)
  std::cout << "max=" << max << ",min=" << min << std::endl;
