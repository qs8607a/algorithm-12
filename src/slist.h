...
#include <iterator>
  ...
  template <typename T> class LinkedList {
   ...
   public:
    inline LinkedList(const LinkedList<T> &l) : d(l.d) {
      d->ref++;
      if (!d->sharable)
        detach();
    }
    ...
    class iterator {
     ...
     public:
      ...
      reference operator*() const { return _M_current->t; }

      pointer operator->() const { return &_M_current->t; }
      ...
      iterator &operator+=(const difference_type &j) {
        return *this = *this + j;
      }

      iterator operator+(const difference_type &j) const {
        Node *n = _M_current;
        if (j > 0) {
          while (j--)
            n = n->n;
      ...
      const Node *base() const { return _M_current; }
    };
