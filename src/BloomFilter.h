...
#include "hash.h"
    template < typename KeyType > class BloomFilter {
 private:
  ...
  static uint32_t BloomHash(const KeyType& key) {
    return slib::Hash(key.data(), key.size(), 0xbc9f1d34);
  }
 ...
 public:
  bool KeyMayMatch(const KeyType& key) {
    const size_t bits = dataLen * bitsPerByte;
  ...
  static BloomFilter* create(const KeyType* keys, int n,
                             int bits_per_key = 10) {
    //计算需要多少位。不能太少，至少64位，否则假阳性率太高
    size_t bits = std::max(64, n * bits_per_key);

    //把位数折合成字节数，此时要向上取整。例如：
    //9位=2字节
    ...
    double ln2_ = 0.69;  // 0.69 =~ ln(2)
                         //k应该在[1，30]之间
    ret->k_ = std::min<size_t>(
        30, std::max<size_t>(1, static_cast<size_t>(bits_per_key * ln2_)));
    ret->init(keys, n);
    return ret;
  }
  ~BloomFilter() throw() { delete[] this->data; }
};
